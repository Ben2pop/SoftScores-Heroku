...
SCALE_HELP_TEXT = _(u"""The scale field is only used
...
class Question(models.Model):
    ...
    multiple_questions_scale = models.TextField(blank=True, null=True, max_length=5000, help_text=SCALE_HELP_TEXT)
    ...
    def get_clean_multiple_scale(self):
        ...
        if self.multiple_questions_scale is None:
            return []
        multiple = []
        for item in self.multiple_questions_scale.split('>>>'):
            item = item.strip()
            if item:
    ...
    def get_multiple_scales(self):
        """
        Parse the scale question field and return a formatted appropriately
        for the 'scale' argument of a customised form widget:
        """
        questions = []
        for item in self.get_clean_multiple_scale():
            questions.append((slugify(item), item))
